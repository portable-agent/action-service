...
public class ApiErrorHandler {
    ...
    ProblemDetail notFound(ActionNotFound error) {
        return problem(HttpStatus.NOT_FOUND, "Action not found", error.getMessage(), "action-not-found");
    }

    @ExceptionHandler({IllegalStateException.class, IllegalArgumentException.class})
    ProblemDetail conflict(RuntimeException error) {
        return problem(HttpStatus.CONFLICT, "Action cannot be changed", error.getMessage(), "action-conflict");
    }
