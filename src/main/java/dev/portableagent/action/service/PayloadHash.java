...
public class PayloadHash {
    public String make(Map<String, Object> payload) {
        try {
            byte[] digest =
                    MessageDigest.getInstance("SHA-256").digest(text(payload).getBytes(StandardCharsets.UTF_8));
    ...
    private String text(Object value) {
        return switch (value) {
            ...
            case List<?> list ->
                list.stream().map(this::text).collect(java.util.stream.Collectors.joining("", "list[", "]"));
