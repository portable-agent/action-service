...
сохраняет его вместе с записью outbox и передаёт идентификатор в Temporal. В первом продуктовом срезе
разрешено только `calendar.create_event` через `fake-calendar`. Настоящие интеграции появятся позже.
